...
const updateTime = new Date(2025, 9, 10, 12, 8); // Lưu ý: tháng 0-11 => 7 = tháng 8

const now = new Date();
const diffMinutes = (now - updateTime) / (1000 * 60); // mili giây → phút

if (diffMinutes >= 0 && diffMinutes <= 20) { //localStorage.setItem("tokenStaff", "user107+01-18");
...
function canUserWatch(storedStr, title) {
  ...
  //   localStorage.setItem("tokenStaff", "user102+01-33");
  // }
  ...
  const allowedList = ["user101", "user102", "user103","user999","user104","user105","user106","user107"];
